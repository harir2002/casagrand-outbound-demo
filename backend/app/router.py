"""Heuristic intent router for demo MVP (swappable with LLM later)."""

from __future__ import annotations

import re

from app.models import Intent, Language, RouteResult
from app.projects import resolve_project_alias
...
def detect_language_switch(text: str) -> Language | None:
    lowered = text.lower()
    # Tanglish first: its hints can contain the substring "tamil".
    ordered = (
        Language.TANGLISH,
        Language.TA,
        Language.EN,
...
def route_utterance(text: str, current_project_id: str) -> RouteResult:
    ...
    # Explicit project switch wins when alias present with switch language.
    if target_project and target_project != current_project_id:
    ...
    if best_intent == Intent.UNKNOWN and target_project:
        return RouteResult(
            intent=Intent.CONTEXT_SWITCH if target_project != current_project_id else Intent.PROJECT_INFO,
            confidence=0.7,
            target_project_id=target_project,
            extracted_slots=extracted,
        )
