"""Versioned Casagrand project knowledge — approved demo facts only."""
...
_PROJECTS: dict[str, ProjectRecord] = {
    "highcity": ProjectRecord(
        ...
        brochure_note=(
            "I can arrange the Highcity brochure and share it after this call "
            "via the sales desk."
        ),
    ),
    "avenuepark": ProjectRecord(
        id="avenuepark",
...
def list_projects() -> list[ProjectSummary]:
    return [
        ProjectSummary(id=p.id, name=p.name, city=p.city, status=p.status)
...
def resolve_project_alias(text: str) -> str | None:
    lowered = text.lower()
    for project_id, terms in PROJECT_ALIASES.items():
